...
class ForecastViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let url = "https://api.openweathermap.org/data/2.5/onecall?lat=41.799688&lon=-88.111088&exclude=minutely,hourly,current ,alerts&units=imperial&appid=89652d405a68b80049a6f3fa42909a1a"
    ...
    private func getDataFromURL(from url:String){
        let task = URLSession.shared.dataTask(with: URL(string: url)!, completionHandler: { data, response, error in
            ...
            //json stuff here
            self.updateDisplay(daily: json.daily)
        })
    ...
    func updateDisplay(daily: [Daily]){
        DispatchQueue.main.async {
//            self.displayLabel.text = "\(daily[0].weather.description)"
